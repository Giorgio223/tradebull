...
# ===== CORS (Vercel + локально) =====
# Важно: allow_origin_regex покроет и preview-домены Vercel типа https://tradebull-xxx.vercel.app
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
...
@app.get("/")
def root():
    # чтобы Render/браузер не показывали 404 на /
    return {"ok": True, "hint": "Use /health /series /docs"}
